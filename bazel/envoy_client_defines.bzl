...
load(
    ...
    "envoy_select_envoy_client_listener",
    "envoy_select_envoy_client_xds",
    "envoy_select_google_grpc",
)

# Compute the defines needed for Envoy Client libraries that don't use Envoy's main library wrappers.
def envoy_client_defines(repository):
    return envoy_select_admin_functionality(["ENVOY_ADMIN_FUNCTIONALITY"], repository) + \
           ...
           envoy_select_envoy_client_listener(["ENVOY_CLIENT_ENABLE_LISTENER"], repository) + \
           envoy_select_envoy_client_xds(["ENVOY_CLIENT_XDS"], repository) + \
