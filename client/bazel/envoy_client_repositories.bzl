...
def upstream_envoy_overrides():
    # Workaround old NDK version breakages https://github.com/envoyproxy/envoy-client/issues/934
    http_archive(
        name = "libevent",
...
def swift_repos():
    ...
    http_archive(
        ...
        patch_args = ["-p1"],
        patches = ["@envoy_client//bazel:xctestrunner.patch"],
    )
...
def kotlin_repos():
    ...
    http_archive(
        ...
        patch_args = ["-p1"],
        patches = ["@envoy_client//bazel:rules_kotlin.patch"],
    )
