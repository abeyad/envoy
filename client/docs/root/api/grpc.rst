...
Envoy Client provides support for gRPC as a thin interface built on top of its :ref:`HTTP APIs <api_http>`.
...
Envoy Client implements the gRPC protocol, accepting and returning serialized protobuf models.

.. note::

  In the future, Envoy Client will provide much more comprehensive integration with gRPC and protobuf,
...
The ``GRPCClient`` type provides the ability to start gRPC streams, and is backed by Envoy Client's
...
Envoy Client provides a ``GRPCRequestHeadersBuilder`` which acts very similarly to the ``RequestHeadersBuilder``
