...
performance using the native networking stacks versus Envoy Client:

- **Control:** Made a request every ``200ms`` to an endpoint without Envoy compiled in the app.
- **Envoy:** Made the same request at the same interval, but using Envoy Client.
...
`this analysis repository <https://github.com/rebello95/EnvoyClientAnalysis/tree/v0.2.3.03062020>`_
...
`release <https://github.com/rebello95/EnvoyClientAnalysis/releases/tag/v0.2.3.03062020>`_.
...
2. Control: ``bazel client-install //examples/kotlin/control:hello_control_kt``
3. Envoy: ``bazel client-install //examples/kotlin/hello_world:hello_envoy_kt --fat_apk_cpu=armeabi-v7a``
...
`perf/cpu label <https://github.com/envoyproxy/envoy-client/labels/perf%2Fcpu>`_.
