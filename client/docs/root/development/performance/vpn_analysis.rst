...
Given the fact that Envoy Client utilizes raw BSD sockets for performing API
...
In order for us to consider Envoy Client to be "working properly" with respect
to VPNs, it needs to:
...
2. Create a mobile app that performs requests to the service using Envoy Client
...
4. Open the app running Envoy Client with a VPN disabled
...
Our understanding of why Envoy Client sends traffic through VPNs the way it
does is as follows.
...
VPN, Envoy Client immediately ends up sending all traffic over the VPN.

However, when a VPN is enabled after the library is already running, any number
of clusters in use by Envoy Client may already have established connections.
...
Interestingly, since Envoy Client typically utilizes 3 clusters (``base``,
...
through Envoy Client would fail before the library recovered and sent traffic
through the non-VPN connection as expected.

Our current understanding of this behavior is that the delay is caused by
the OS destroying the VPN connection immediately, at which point Envoy Client
allows several failures before tearing down its connection and estabilshing
a new one.

With trace logs enabled, we saw Envoy Client returning some local ``503``
...
This behavior suggests that we could further optimize Envoy Client's behavior
...
switch traffic onto the VPN slightly faster than Envoy Client. This can likely
...
Based on the above investigations, Envoy Client handles VPN connections
