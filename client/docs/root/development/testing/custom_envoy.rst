...
information or verify that an upstream change will work well for Envoy Client.
...
that the Envoy Client build bots will pass.

To test Envoy changes against CI, make those changes in your Envoy repo, push that branch to GitHub
and change the ``.gitmodules`` file in the Envoy Client repo to point to your Envoy branch like so::
