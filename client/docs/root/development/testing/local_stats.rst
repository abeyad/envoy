...
The `local-stats` `branch <https://github.com/envoyproxy/envoy-client/tree/local-stats>`_ allows a
...
The `config template <https://github.com/envoyproxy/envoy-client/blob/local-stats/library/common/config_template.cc>`_
has already been updated to use a local statsd server. However, if you are using Android to test,
the `static address <https://github.com/envoyproxy/envoy-client/blob/local-stats/library/common/config_template.cc#L203>`_
...
2. Start statsd locally. You can do this by cloning `statsd <https://github.com/statsd/statsd>`_ and running from within the root of
  ...
  An example ``config.js`` file. Note that the port must match the port in the `config_template <https://github.com/envoyproxy/envoy-client/blob/local-stats/library/common/config_template.cc#L203>`_::
