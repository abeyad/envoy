...
The Envoy Client proposed roadmap has similarities to several existing systems. They are compared
below.
...
Envoy Client targeted at advanced client networking protocols, with the caveat that it already
supports QUIC. Envoy Client's goals are larger and include extensibility, observability, IDL
...
code has been open sourced. Like Cronet, Envoy Client's goals are a superset of what proxygen and
mvfst provide.
...
closer to Envoy Client than Cronet or proxygen/mvfst, and in fact gRPC is currently in the process
...
possible that in the future gRPC's mobile clients will continue to converge with Envoy Client, but
in the near term we feel that Envoy Client can deliver a production ready iOS/Android feature set
that includes xDS, QUIC, etc. in a shorter time frame. Note that Envoy Client will likely continue
