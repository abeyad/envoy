...
- Envoy Client's release builds compile without admin support by default. (``--define=admin_functionality=disabled``) (:issue`#2693 <2693>`)
...
- clusters: removing the base_h2 cluster. Requests with ``x-envoy-client-upstream-protocol`` set to ``http2`` will be sent to the base cluster and use the best available protocol (:issue `#25796 <25796>`).
...
- kotlin: add a way to tell Envoy Client to respect system proxy settings by calling an ``enableProxying(true)`` method on the engine builder. (:issue:`#2416 <2416>`)
...
- build: Add a build feature ``exclude_certificates`` to disable inclusion of the Envoy Client certificate list, for use when using platform certificate validation.
...
- iOS: add support for integrating Envoy Client via the Swift Package Manager
...
- HTTP: Switching to Envoy Client HCM (:issue:`#1716 <1716>`)
...
In the last few months the team has continued to harden Envoy Client with production exposure.
...
Lastly, and perhaps most importantly, we have adopted a formal `inclusive language policy <https://github.com/envoyproxy/envoy-client/blob/main/CONTRIBUTING.md#inclusive-language-policy>`_
(:issue:`#948 <948>`) and updated all necessary locations (:issue:`#944 <944>`, :issue:`#945 <945>`, :issue:`#946 <946>`)

0.3.0 (Mar 26, 2020)
====================

This is the first release of Envoy Client Lyft is using in a production application! 🎉
...
binaries with Envoy Client for a couple weeks now, and are starting to expose a percentage of our
production clients to Envoy Client with this release.
...
- Fixes DNS resolution when starting Envoy Client offline on iOS (:issue:`#672 <672>`)
...
Envoy Client v0.2.2 changes how network requests are performed to no longer use Envoy's `AsyncClient` and to instead consume the `ApiListener` directly (:issue:`#616 <616>`).

Additional changes:

- Domain specification when starting the library is no longer supported (:issue:`#641 <641>`, :issue:`#642 <642>`). Envoy Client now uses the authority specified when starting a new stream
- Less aggressive retry back-off policies (:issue:`#652 <652>`)

0.2.1 (Jan 6, 2020)
===================

This release of Envoy Client contains some small improvements:
...
Envoy Client v0.2 is a fundamental shift in how mobile clients use Envoy. Envoy Client now provides native Swift/Kotlin APIs that call through to Envoy directly (rather than using Envoy as a proxy), which apps use to create and interact with network streams.
