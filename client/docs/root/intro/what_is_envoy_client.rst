What is Envoy Client?
=====================

.. attention::

  Envoy Client is currently in a very early stage of development. This page should be read as a set
  of aspirational project goals.

Envoy Client is an iOS and Android client networking library built using
...
applications that are so critical to the end user’s experience. Envoy Client in conjunction with
...
polyglot architecture, Envoy Client is distributed as a library meant to be compiled directly into
...
`protocol buffers <https://developers.google.com/protocol-buffers/>`_, Envoy Client will standardize
...
With protocol buffer’s powerful annotation/extension system, Envoy Client can add sophisticated
...
Furthermore, it is an explicit goal of Envoy Client to maintain consistent 1:1 APIs between
...
service API. This API will allow both Envoy and Envoy Client to fetch generic protobuf definitions
from a central IDL service, which can then be used to implement annotation driven networking via
reflection. This model means that Envoy Client will not necessarily need to have prior knowledge of
...
of the long term goals of Envoy Client is to bring xDS configuration all the way to mobile clients,
