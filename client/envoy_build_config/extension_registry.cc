...
#ifdef ENVOY_CLIENT_ENABLE_LISTENER
...
void ExtensionRegistry::registerFactories() {
  ...
  // This is the default cluster used by envoy client to establish connections upstream.
  ...
  // This filter handles client-specific network config like interface binding and system proxies.
  ...
  // This filter applies socket tagging based on the x-envoy-client-socket-tag header.
  ...
  // Envoy Client uses the GetAddrInfo resolver for DNS lookups on android by default.
...
#ifdef ENVOY_CLIENT_ENABLE_LISTENER
  // These are downstream factories required if Envoy Client is compiled with
  // proxy functionality.
...
#ifdef ENVOY_CLIENT_ENABLE_LISTENER
  // These are QUIC downstream factories required if Envoy Client is compiled with
...
#ifdef ENVOY_CLIENT_XDS
  // These extensions are required for xDS over gRPC using ADS, which is what Envoy Client
  // supports for xDS.
