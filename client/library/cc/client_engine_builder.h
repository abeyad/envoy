...
// created from.
class ClientEngineBuilder : public Platform::EngineBuilderBase<ClientEngineBuilder> {
public:
  ClientEngineBuilder();
  ...
  ClientEngineBuilder& addConnectTimeoutSeconds(int connect_timeout_seconds);
  ClientEngineBuilder& addDnsRefreshSeconds(int dns_refresh_seconds);
  ClientEngineBuilder& addDnsFailureRefreshSeconds(int base, int max);
  ClientEngineBuilder& addDnsQueryTimeoutSeconds(int dns_query_timeout_seconds);
  ...
  ClientEngineBuilder& addDnsMinRefreshSeconds(int dns_min_refresh_seconds);
  ClientEngineBuilder& setDnsNumRetries(uint32_t dns_num_retries);
  ClientEngineBuilder& setGetaddrinfoNumThreads(uint32_t num_threads);
  ClientEngineBuilder& addMaxConnectionsPerHost(int max_connections_per_host);
  ClientEngineBuilder& addH2ConnectionKeepaliveIdleIntervalMilliseconds(
  ...
  ClientEngineBuilder& setAppVersion(std::string app_version);
  ClientEngineBuilder& setAppId(std::string app_id);
  ClientEngineBuilder& setDeviceOs(std::string app_id);

  ClientEngineBuilder& setPerTryIdleTimeoutSeconds(int per_try_idle_timeout_seconds);
  ClientEngineBuilder& enableGzipDecompression(bool gzip_decompression_on);
  ClientEngineBuilder& enableBrotliDecompression(bool brotli_decompression_on);
  ClientEngineBuilder& enableSocketTagging(bool socket_tagging_on);
  ClientEngineBuilder& enableHttp3(bool http3_on);
  ClientEngineBuilder& enableEarlyData(bool early_data_on);
  ClientEngineBuilder& enableScone(bool enable);
  ClientEngineBuilder& addQuicConnectionOption(std::string option);
  ClientEngineBuilder& addQuicClientConnectionOption(std::string option);
  // Deprecated, use addQuicConnectionOption() instead.
  ClientEngineBuilder& setHttp3ConnectionOptions(std::string options);
  // Deprecated, use addQuicClientConnectionOption() instead.
  ClientEngineBuilder& setHttp3ClientConnectionOptions(std::string options);
  ClientEngineBuilder& addQuicHint(std::string host, int port);
  ClientEngineBuilder& addQuicCanonicalSuffix(std::string suffix);
  // 0 means port migration is disabled.
  ClientEngineBuilder& setNumTimeoutsToTriggerPortMigration(int num_timeouts);
  ClientEngineBuilder& enableInterfaceBinding(bool interface_binding_on);
  ClientEngineBuilder& enableDrainPostDnsRefresh(bool drain_post_dns_refresh_on);
  ClientEngineBuilder& setUdpSocketReceiveBufferSize(int32_t size);
  ClientEngineBuilder& setUdpSocketSendBufferSize(int32_t size);
  ClientEngineBuilder& enforceTrustChainVerification(bool trust_chain_verification_on);
  ClientEngineBuilder& setUpstreamTlsSni(std::string sni);
  ClientEngineBuilder&
  ...
  // thread.
  ClientEngineBuilder& enableWorkerThread(bool use_worker_thread);
  ...
  ClientEngineBuilder& enableDnsCache(bool dns_cache_on, int save_interval_seconds = 1);
  // Set additional socket options on the upstream cluster outbound sockets.
  ClientEngineBuilder& setAdditionalSocketOptions(
  ...
  ClientEngineBuilder& addNativeFilter(std::string name, std::string typed_config);
  ClientEngineBuilder& addNativeFilter(const std::string& name, const Protobuf::Any& typed_config);

  ClientEngineBuilder& addPlatformFilter(const std::string& name);
  ...
  ClientEngineBuilder& setMaxConcurrentStreams(int max_concurrent_streams);
  ...
  // Sets the xDS configuration for the Envoy Client engine.
  //
  // `xds_builder`: the XdsBuilder instance used to specify the xDS configuration options.
  ClientEngineBuilder& setXds(XdsBuilder xds_builder);
  ...
  // value will be used.
  ClientEngineBuilder& respectSystemProxySettings(bool value, int refresh_interval_secs = 10);
  ClientEngineBuilder& setIosNetworkServiceType(int ios_network_service_type);
#endif
  // Overload to preserve the same return type as the EngineBuilder class.
  EngineSharedPtr build();

private:
  friend class Platform::EngineBuilderBase<ClientEngineBuilder>;
  ...
  // EVMB is to distinguish Envoy Client client connections.
...
using MobileEngineBuilderSharedPtr = std::shared_ptr<ClientEngineBuilder>;
