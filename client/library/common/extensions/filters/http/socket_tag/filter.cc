...
Http::FilterHeadersStatus SocketTagFilter::decodeHeaders(Http::RequestHeaderMap& request_headers,
                                                         bool) {
  static auto socket_tag_header = Http::LowerCaseString("x-envoy-client-socket-tag");
  Http::RequestHeaderMap::GetResult header = request_headers.get(socket_tag_header);
  if (header.empty()) {
    return Http::FilterHeadersStatus::Continue;
  }

  // The x-envoy-client-socket-tag header must contain a pair of number separated by a comma, e.g.:
  // x-envoy-client-socket-tag: 123,456
  ...
  if (!absl::SimpleAtoi(data.first, &uid) || !absl::SimpleAtoi(data.second, &traffic_stats_tag)) {
    decoder_callbacks_->sendLocalReply(
        ...
        absl::StrCat("Invalid x-envoy-client-socket-tag header: ", tag_string), nullptr,
        absl::nullopt, "");
