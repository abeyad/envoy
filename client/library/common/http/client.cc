...
/**
 * IMPORTANT: stream closure semantics in envoy client depends on the fact that the HCM fires a
...
Client::DirectStream::registerCodecEventCallbacks(CodecEventCallbacks* codec_callbacks) {
  ...
  // Envoy stream is going away. Make sure Envoy Client sees the stream as
  // closed as well.
...
void Client::DirectStream::dumpState(std::ostream&, int indent_level) const {
  // TODO(junr03): output to ostream arg - https://github.com/envoyproxy/envoy-client/issues/1497.
  std::stringstream ss;
...
void Client::startStream(envoy_stream_t new_stream_handle, EnvoyStreamCallbacks&& stream_callbacks,
  ...
  // Note: streams created by Envoy Client are tagged as is_internally_created. This means that
...
void Client::sendHeaders(envoy_stream_t stream, RequestHeaderMapPtr headers, bool end_stream,
  ...
  // from the caller.
  // https://github.com/envoyproxy/envoy-client/issues/301
  if (!direct_stream) {
    return;
  }
  ...
  // Set the x-forwarded-proto header to https because Envoy Client only has clusters with TLS
  ...
  //
  // Setting this header is also currently important because Envoy Client starts stream with the
  // ApiListener setting the is_internally_created bool to true. This means the
  // Http::ConnectionManager *will not* mutate Envoy Client's request headers. One of the
...
const LowerCaseString ClusterHeader{"x-envoy-client-cluster"};
