...
// There is only one shared ClientProcessWide instance for all Envoy Client engines.
ClientProcessWide& initOnceClientProcessWide(const OptionsImplBase& options) {
  MUTABLE_CONSTRUCT_ON_FIRST_USE(ClientProcessWide, options);
}
...
envoy_status_t InternalEngine::run(std::shared_ptr<OptionsImplBase> options) {
  initOnceClientProcessWide(*options);
