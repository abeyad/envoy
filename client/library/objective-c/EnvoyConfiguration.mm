...
@implementation ECOHeaderMatcher
...
@implementation ECORouteMatcher
- (Envoy::DirectResponseTesting::RouteMatcher)toCXX {
  ...
  for (ECOHeaderMatcher *matcher in self.headers) {
...
@implementation ECODirectResponse
...
- (ClientEngineBuilder)applyToCXXBuilder {
...
- (std::unique_ptr<envoy::config::bootstrap::v3::Bootstrap>)generateBootstrap {
  try {
    ClientEngineBuilder builder = [self applyToCXXBuilder];
