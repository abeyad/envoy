# Envoy Client Python Library

This library provides Python bindings for [Envoy Client](https://envoyclient.io/), allowing Python applications to leverage Envoy's high-performance networking capabilities.
...
The `AsyncClient` is the recommended way to use Envoy Client in Python.

```python
import asyncio
from envoy_client import AsyncClient, EngineBuilder, LogLevel
...
async def main():
    ...
    async with AsyncClient(builder) as client:
        ...
        response = await client.request(
            method="GET",
            url="https://api.github.com/repos/envoyproxy/envoy-client"
        )
...
```python
from envoy_client import EngineBuilder, LogLevel
...
See the [Envoy Client contribution guide](https://github.com/envoyproxy/envoy-client/blob/main/CONTRIBUTING.md) for details on how to contribute to this project.
