"""Utilities for mapping httpx requests and responses to Envoy Client."""
...
def get_envoy_headers(
    ...
    for key, values in norm_headers.items():
        ...
        # Envoy Client automatically sets the correct upstream authority/host headers.
        if key.startswith(":") or key.lower() == "host":
            continue
        # Avoid HTTP/1.1 connection-specific headers, which are forbidden in HTTP/2.
        # Envoy Client will handle and set corresponding upstream connection headers properly on its own.
        if key.lower() in {
            "connection",
            "keep-alive",
            "proxy-connection",
    ...
    if socket_tag is not None:
        header_dict["x-envoy-client-socket-tag"] = f"0,{socket_tag}"
