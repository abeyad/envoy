...
class EnvoyClientMetricsServiceIntegrationTest
    : public Grpc::VersionedGrpcClientIntegrationParamTest,
      public HttpIntegrationTest {
public:
  EnvoyClientMetricsServiceIntegrationTest()
  ...
  void initialize() override {
    config_helper_.addConfigModifier([this](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
      ...
      envoyclient::extensions::stat_sinks::metrics_service::EnvoyClientMetricsServiceConfig config;
  ...
  AssertionResult waitForMetricsRequest() {
    ...
    while (!(known_counter_exists && known_gauge_exists && known_histogram_exists)) {
      ...
      EXPECT_EQ("/envoyclient.extensions.stat_sinks.metrics_service.EnvoyClientMetricsService/"
                "EnvoyClientStreamMetrics",
...
INSTANTIATE_TEST_SUITE_P(IpVersionsClientType, EnvoyClientMetricsServiceIntegrationTest,
                         VERSIONED_GRPC_CLIENT_INTEGRATION_PARAMS);

// Test a basic metric service flow.
TEST_P(EnvoyClientMetricsServiceIntegrationTest, BasicFlow) {
  initialize();
