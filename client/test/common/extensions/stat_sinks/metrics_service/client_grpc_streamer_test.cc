...
namespace StatSinks {
namespace EnvoyClientMetricsService {
namespace {

class EnvoyClientGrpcMetricsStreamerImplTest : public testing::Test {
public:
  using MockMetricsStream = Grpc::MockAsyncStream;
  using MetricsServiceCallbacks = Grpc::AsyncStreamCallbacks<
      envoyclient::extensions::stat_sinks::metrics_service::EnvoyClientStreamMetricsResponse>;

  EnvoyClientGrpcMetricsStreamerImplTest() {
    EXPECT_CALL(*factory_, create()).WillOnce(Invoke([this] {
      return Grpc::RawAsyncClientPtr{async_client_};
    }));

    streamer_ = std::make_unique<EnvoyClientGrpcMetricsStreamerImpl>(
  ...
  EnvoyClientGrpcMetricsStreamerImplPtr streamer_;
};

// Test basic metrics streaming flow.
TEST_F(EnvoyClientGrpcMetricsStreamerImplTest, BasicFlow) {
  InSequence s;
  ...
  callbacks1->onReceiveMessage(
      std::make_unique<envoyclient::extensions::stat_sinks::metrics_service::
                           EnvoyClientStreamMetricsResponse>());
}

// Test response is received.
TEST_F(EnvoyClientGrpcMetricsStreamerImplTest, WithResponse) {
  InSequence s;
  ...
  EXPECT_LOG_CONTAINS("debug", "EnvoyClient streamer received batch_id: mock-batch-id",
                      callbacks1->onReceiveMessage(std::move(response)));
}

// Test that stream failure is handled correctly.
TEST_F(EnvoyClientGrpcMetricsStreamerImplTest, StreamFailure) {
  InSequence s;
...
class MockGrpcMetricsStreamer
    : MetricsService::GrpcMetricsStreamer<
          envoyclient::extensions::stat_sinks::metrics_service::EnvoyClientStreamMetricsMessage,
          envoyclient::extensions::stat_sinks::metrics_service::EnvoyClientStreamMetricsResponse> {
public:
  // GrpcMetricsStreamer
  MOCK_METHOD(void, send, (MetricsService::MetricsPtr && metrics));
  MOCK_METHOD(void, onReceiveMessage,
              (std::unique_ptr<envoyclient::extensions::stat_sinks::metrics_service::
                                   EnvoyClientStreamMetricsResponse> &&
               response));
};
