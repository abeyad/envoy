...
// Use the envoy client default config as much as possible in this test.
...
BaseClientIntegrationTest::BaseClientIntegrationTest(Network::Address::IpVersion ip_version)
  ...
  full_dispatcher_ = api_->allocateDispatcher("fake_envoy_client");
  use_lds_ = false;
  ...
  // interface gets compiled out by default in Envoy Client, remove it from the ConfigHelper's
  // bootstrap config.
