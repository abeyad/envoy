  ...
  fun `cannot publicly add headers with restricted prefix`() {
    val headers =
      RequestHeadersBuilder(
        ...
        .add(":x-foo", "123")
        .add("x-envoy-client-foo", "abc")
        .add("host", "example.com")
        .add("hostWithSuffix", "foo.bar")
        .build()

    assertThat(headers.caseSensitiveHeaders()).doesNotContainKey(":x-foo")
    assertThat(headers.caseSensitiveHeaders()).doesNotContainKey("x-envoy-client-foo")
  ...
  fun `cannot publicly set headers with restricted prefix`() {
    val headers =
      RequestHeadersBuilder(
        ...
        .set("x-envoy-client-foo", mutableListOf("abc"))
        .build()
  ...
  fun `can internally set headers with restricted prefix`() {
    val headers =
      RequestHeadersBuilder(
        ...
        .internalSet("x-envoy-client-foo", mutableListOf("abc"))
        .build()

    assertThat(headers.value(":x-foo")).containsExactly("123")
    assertThat(headers.value("x-envoy-client-foo")).containsExactly("abc")
  }
