...
These tests use unittest.mock to simulate the Envoy Client C++ engine (envoy_engine).
...
# The code does `from .. import envoy_engine` from inside envoy_client.async_client_transport
# So it looks for `envoy_client.envoy_engine`
sys.modules["envoy_client.envoy_engine"] = mock_envoy_engine

import httpx
from envoy_client import (
...
class TestEnvoyClientTransport(unittest.TestCase):
    ...
    def test_handle_request_socket_tag(self):
        ...
        self.assertIn("x-envoy-client-socket-tag", sent_headers)
        self.assertEqual(sent_headers["x-envoy-client-socket-tag"], expected_header_value)
...
from envoy_client.async_client.executor import AsyncioExecutor
